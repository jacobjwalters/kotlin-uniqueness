...
#mathpar(
  ...
  proof-tree(rule(
    name: "BinOp",
    typeExpr($Gamma$, $e_1 plus.o e_2$, $tau_3$),
    typeExpr($Gamma$, $e_1$, $tau_1$),
    typeExpr($Gamma$, $e_2$, $tau_2$),
    $plus.o : tau_1 times tau_2 -> tau_3$,
  )),
  proof-tree(rule(
    name: "UnOp",
    typeExpr($Gamma$, $plus.o (e)$, $tau_2$),
    typeExpr($Gamma$, $e$, $tau_1$),
    $plus.o : tau_1 -> tau_2$,
  )),

  proof-tree(rule(
    name: "IfExpr",
    typeExpr($Gamma$, $#If e #Then e_1 #Else e_2$, $tau$),
    typeExpr($Gamma$, $e$, $#Bool$),
    typeExpr($Gamma$, $e_1$, $tau$),
    typeExpr($Gamma$, $e_2$, $tau$),
  )),

  proof-tree(rule(
    name: "WhileExpr",
    typeExpr($Gamma$, $#While c brace.l e brace.r$, $#Unit$),
    typeExpr($Gamma$, $c$, $#Bool$),
    typeExpr($Gamma$, $e$, $#Unit$),
  )),
  ...
  proof-tree(rule(
    name: "ScopeExpr",
    typeExpr($Gamma$, $#Scope brace.l s; e brace.r$, $tau$),
    typeStmt($Gamma$, $s$, $Gamma'$),
    typeExpr($Gamma'$, $e$, $tau$),
  )),
)
...
#mathpar(
  proof-tree(rule(name: "VarDecl", typeStmt($Gamma$, $#Var x : tau = e$, $Gamma, x : tau$), typeExpr(
    $Gamma$,
    $e$,
    $tau$,
  ))),
  proof-tree(rule(name: "VarAssign", typeStmt($Gamma$, $x = e$, $Gamma$), $Gamma(x) = tau$, typeExpr(
    $Gamma$,
    $e$,
    $tau$,
  ))),

  proof-tree(rule(name: "Seq", typeStmt($Gamma$, $s_1; s_2$, $Gamma''$), typeStmt($Gamma$, $s_1$, $Gamma'$), typeStmt(
    $Gamma'$,
    $s_2$,
    $Gamma''$,
  ))),
...
#mathpar(
  proof-tree(rule(
    name: $#IfCondK$,
    typeContE($Gamma$, $#ifCondK (e_1, e_2) dot.c K$, $#Bool$),
    typeExpr($Gamma$, $e_1$, $tau$),
    typeExpr($Gamma$, $e_2$, $tau$),
    typeContE($Gamma$, $K$, $tau$),
  )),

  proof-tree(rule(name: $#DeclK$, typeContE($Gamma$, $#declK (x : tau) dot.c K$, $tau$), typeContC(
    $Gamma, x : tau$,
    $K$,
  ))),
  proof-tree(rule(name: $#AssignK$, typeContE($Gamma$, $#assignK (x) dot.c K$, $tau$), $Gamma(x) = tau$, typeContC(
    $Gamma$,
    $K$,
  ))),
  ...
  proof-tree(rule(
    name: $#BinOpLK$,
    typeContE($Gamma$, $#binopLK (plus.o, e_2) dot.c K$, $tau_1$),
    $plus.o : tau_1 times tau_2 -> tau_3$,
    typeExpr($Gamma$, $e_2$, $tau_2$),
    typeContE($Gamma$, $K$, $tau_3$),
  )),
  proof-tree(rule(
    name: $#BinOpRK$,
    typeContE($Gamma$, $#binopRK (plus.o, v_1) dot.c K$, $tau_2$),
    $plus.o : tau_1 times tau_2 -> tau_3$,
    $tack.r v_1 : tau_1$,
    typeContE($Gamma$, $K$, $tau_3$),
  )),
  proof-tree(rule(
    name: $#UnOpK$,
    typeContE($Gamma$, $#unopK (plus.o) dot.c K$, $tau_1$),
    $plus.o : tau_1 -> tau_2$,
    typeContE($Gamma$, $K$, $tau_2$),
  )),

  proof-tree(rule(
    name: $#LoopK$,
    typeContE($Gamma$, $#loopK (c, e, n) dot.c K$, $#Bool$),
    typeExpr($Gamma$, $c$, $#Bool$),
    typeExpr($Gamma$, $e$, $#Unit$),
    typeContE($#truncate($Gamma$, $n$)$, $K$, $#Unit$),
  )),
  proof-tree(rule(
    name: $#LoopContK$,
    typeContE($Gamma$, $#loopContK (c, e, n) dot.c K$, $#Unit$),
    typeExpr($Gamma$, $c$, $#Bool$),
    typeExpr($Gamma$, $e$, $#Unit$),
    typeContE($#truncate($Gamma$, $n$)$, $K$, $#Unit$),
  )),

  proof-tree(rule(name: $#ScopeExitK$, typeContE($Gamma$, $#scopeExitK (n) dot.c K$, $tau$), typeContE(
    $#truncate($Gamma$, $n$)$,
    $K$,
    $tau$,
  ))),
...
#mathpar(
  ...
  proof-tree(rule(name: $#SeqK$, typeContC($Gamma$, $#seqK (s) dot.c K$), typeStmt($Gamma$, $s$, $Gamma'$), typeContC(
    $Gamma'$,
    $K$,
  ))),
  proof-tree(rule(
    name: $#ScopeBodyK$,
    typeContC($Gamma$, $#scopeBodyK (e, n) dot.c K$),
    typeExpr($Gamma$, $e$, $tau$),
    typeContE($#truncate($Gamma$, $n$)$, $K$, $tau$),
  )),
)
...
#mathpar(
  ...
  proof-tree(rule(
    name: "WtContV",
    $tack.r #cekC($v$, $E$, $K$) "ok"$,
    $#coh($E$, $Gamma$)$,
    $tack.r v : tau$,
    typeContE($Gamma$, $K$, $tau$),
  )),
  proof-tree(rule(name: "WtContS", $tack.r #cekC($#Skip$, $E$, $K$) "ok"$, $#coh($E$, $Gamma$)$, typeContC(
    $Gamma$,
    $K$,
  ))),
)
